...
func (r *AuthPostgres) CreateOrg(org packom.Org) (int, error) {
	var o_id int
	query := `insert into public."Orgs" (o_id, login, hashed_pwd, name, group_id, status, site, phone, email, adress, info, history) values (default, $1, $2, $3, $4, false, '', '', '', '', '', '') returning o_id`
	...
	query = `insert into public."Orgs_specs" (o_id, spec_id) values ($1, 0) returning o_id`
	...
	query = `insert into public."Org_countries" (o_id, country_id) values ($1, 0) returning o_id`
